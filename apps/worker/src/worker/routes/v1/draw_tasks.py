...
def register_draw_task_routes(router: APIRouter, draw_tasks: DrawTaskService) -> None:
    ...
    async def create_draw_task(request: Request) -> Response:
        ...
        return JSONResponse({"task": with_public_result_url(task)}, status_code=201)
    ...
    def get_draw_task(task_id: str) -> Response:
        ...
        return JSONResponse({"task": with_public_result_url(task)})


def with_public_result_url(task: dict[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(result_filename, str):
        return {**public_task, "resultUrl": None}

    if is_public_url(result_filename):
        return {**public_task, "resultUrl": result_filename}

    return {**public_task, "resultUrl": generated_image_public_path(result_filename)}
