...
当前任务模型还支持基于已生成图片继续出图：

- `parentAssetId`：声明本次任务引用的来源图片。
- `branchMode`：声明继续方式，取值为 `evolve`、`preserve`、`transform`。

这两个字段会原样存档在任务记录中；真正发给图像模型的执行提示词会在 worker 侧按模式编译，前端历史里仍保留用户原始提示词。
